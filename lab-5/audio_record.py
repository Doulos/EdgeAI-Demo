# Description :  Code to acquire 5s audio clips and converts to Spectrogram
...
for i in range(6):
        arecord_cli = (f'arecord --format=S16_LE  --rate=16000 --channels=1 --device=hw:2,0 --duration 5  waves{i}.wav')
        subprocess.run(args = arecord_cli, shell=True)
        print (f' Record Iteration number {i}')

for i in range (6):
        sox_cli =(f'sox waves{i}.wav -n rate 16k spectrogram -x 224 -y 224 -q 5 -o waves{i}.png')
